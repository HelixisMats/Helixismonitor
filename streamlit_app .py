...
with tab_hist:
    ...
    with col_mode:
        range_mode = st.radio("Välj intervall", ["Snabbval", "Datumval"], horizontal=True)

    if range_mode == "Snabbval":
        with col_from:
            hours = st.selectbox("Tidsintervall",
                [1, 6, 12, 24, 48, 168, 336, 720],
                index=3,
        ...
        with st.spinner(T["loading_hist"]):
            ...
            df_smhi_h = fetch_smhi_history(hours)
    else:
        min_date = datetime(2026, 1, 1).date()
        max_date = datetime.now(SWE).date()
        with col_from:
            date_from = st.date_input("Från", value=max_date - timedelta(days=7),
                                      min_value=min_date, max_value=max_date)
        with col_to:
            date_to = st.date_input("Till", value=max_date,
                                    min_value=min_date, max_value=max_date)
        if date_from > date_to:
            st.error("Från-datum måste vara före till-datum.")
            st.stop()
        ...
        hours   = max(24, int((dt_to - dt_from).total_seconds() / 3600) + 1)
        with st.spinner(T["loading_hist"]):
