...
@st.cache_data(ttl=30)
def fetch_today():
    # Swedish midnight — correct "today" for local time
...
def make_thermo(label, val, mn, mx, color):
    ...
    fig.add_shape(type="circle",
        x0=-0.35, x1=0.35, y0=mn-8, y1=mn+8,
...
# At night the system is idle — only flag red if no data for >15 min
is_live  = age_min < 15
...
<div style=\'padding-top:14px\'>
  ...
  <span class=\"ts-text\">{last_swe.strftime(\'%H:%M:%S\')}</span>
</div>""", unsafe_allow_html=True)
with h3:
...
if st.session_state.view == "gauges":
    ...
    with t3:
        fig_d = go.Figure()
        for s,color,yax,name in [("irradiance",AMBER,"y","W/m²"),("power",RUST,"y2","kW")]:
