...
def fetch_data(hours):
    ...
    try:
        res = db.table("sensor_readings").select("created_at,sensor,value")             .gte("created_at", since).order("created_at").limit(50_000).execute()
...
def fetch_today():
    ...
    try:
        res = db.table("sensor_readings").select("created_at,sensor,value")             .gte("created_at", today_start.isoformat()).order("created_at").limit(20_000).execute()
    except Exception:
        return pd.DataFrame()
    if not res.data: return pd.DataFrame()
    df = pd.DataFrame(res.data)
...
# ── SVG Thermometer ───────────────────────────────────────────
def svg_thermometer(label, val, mn, mx, color_hex):
    """Classic thermometer: bulb at bottom, tube above, graduated."""
    ...
    bulb_cy = H - bulb_r - 4
    tube_top = 16
    tube_bot = bulb_cy - bulb_r + 6
    tube_h = tube_bot - tube_top

    fill_h = pct * tube_h
    fill_top = tube_bot - fill_h

    mid_val = round((mn + mx) / 2)

    # Tick marks
    ticks = ""
    for tv in [mn, mid_val, mx]:
        tp = (tv - mn) / (mx - mn)
        ty = tube_bot - tp * tube_h
        ticks += f"""
        <line x1="{cx + tube_w//2}" y1="{ty}" x2="{cx + tube_w//2 + 6}" y2="{ty}"
              stroke="{MUTED}" stroke-width="1"/>
        <text x="{cx + tube_w//2 + 9}" y="{ty + 4}" font-size="8"
              fill="{MUTED}" font-family="Inter">{tv}°</text>"""

    svg = f"""
<div style="text-align:center; padding:4px 0;">
  <div style="font-size:10px;font-weight:600;color:{TEXT};font-family:Inter;margin-bottom:2px">{label}</div>
  <svg width="{W}" height="{H}" viewBox="0 0 {W} {H}" xmlns="http://www.w3.org/2000/svg">
    <!-- Tube background -->
    <rect x="{tube_x}" y="{tube_top}" width="{tube_w}" height="{tube_h}"
          rx="7" fill="{LGRAY}" stroke="{BORDER}" stroke-width="1"/>
    <!-- Tube fill -->
    <rect x="{tube_x + 2}" y="{fill_top}" width="{tube_w - 4}" height="{fill_h}"
          rx="5" fill="{color_hex}"/>
    <!-- Bulb background -->
    <circle cx="{cx}" cy="{bulb_cy}" r="{bulb_r}" fill="{LGRAY}" stroke="{BORDER}" stroke-width="1"/>
    <!-- Bulb fill -->
    <circle cx="{cx}" cy="{bulb_cy}" r="{bulb_r - 3}" fill="{color_hex}"/>
    <!-- Connect tube to bulb -->
    <rect x="{tube_x + 2}" y="{tube_bot - 4}" width="{tube_w - 4}" height="10" fill="{color_hex}"/>
    <!-- Tick marks -->
    {ticks}
    <!-- Value label -->
    <text x="{cx}" y="{max(fill_top - 4, tube_top + 14)}"
          text-anchor="middle" font-size="12" font-weight="700"
          fill="{color_hex}" font-family="Inter">{display:.1f}°</text>
  </svg>
</div>"""
    return svg
...
def semi(label, val, mn, mx, unit, color, sub_text="", warn=None):
    ...
    # Format number
    nfmt = ".2f" if unit in ["bar", "m³/h"] else ".1f"
    if unit == "W/m²": nfmt = ".0f"
    ...
    fig = go.Figure(go.Indicator(
        ...
        title={
            "text": (f"<span style='font-weight:600;font-size:13px;"
                     f"color:{TEXT};font-family:Inter'>{label}</span>"
                     f"<br><span style='font-size:10px;color:{MUTED};"
                     f"font-family:Inter'>{sub_text}</span>"),
            "font": {"size": 13, "family": "Inter"},
        },
    ))
    fig.update_layout(
        height=230,
        margin=dict(l=24, r=24, t=80, b=12),
...
<div style='padding-top:10px'>
  ...
  <span class="ts-text">{last_ts.astimezone(SWE).strftime('%H:%M:%S')}</span>
</div>""", unsafe_allow_html=True)
with h3:
...
if st.session_state.view == "gauges":
    ...
    tc = st.columns(5)
    thermo_cfg = [
        ("Collector R", "temp_right_coll", "#A83030", 20, 160),
        ("Collector L", "temp_left_coll",  "#B87200", 20, 160),
        ("Forward",     "temp_forward",    "#A83030", 20, 120),
        ("Return",      "temp_return",     "#2E5EA0", 10, 100),
        ("Tank",        "temp_tank",       "#167A5E", 10, 100),
    ]
    for col, (lbl, sensor, color, mn, mx) in zip(tc, thermo_cfg):
        col.markdown(svg_thermometer(lbl, v.get(sensor), mn, mx, color),
                     unsafe_allow_html=True)
    ...
    with f3:
        ...
        irr_sub = "Excellent" if (irr and irr > 700)                   else ("Moderate" if (irr and irr > 200) else "Low — limited output")
    ...
    with t1:
        st.plotly_chart(linechart(chart_df,
            ...
            ["#A83030","#B87200","#C06020","#2E5EA0","#167A5E"], "°C"),
            use_container_width=True)
    with t2:
        ca, cb = st.columns(2)
        with ca:
            st.caption("Thermal power (kW)")
            st.plotly_chart(linechart(chart_df,["power"],["#A83030"],"kW",240), use_container_width=True)
        with cb:
            st.caption("Flow rate (m³/h)")
            st.plotly_chart(linechart(chart_df,["flow"],["#2E5EA0"],"m³/h",240), use_container_width=True)
    with t3:
        st.caption("Irradiance vs thermal output — strong correlation = efficient tracking")
        fig_d = go.Figure()
        for s,color,yax,name in [
            ("irradiance","#B87200","y","Irradiance (W/m²)"),
            ("power","#A83030","y2","Power (kW)"),
        ]:
        ...
        fig_d.update_layout(height=270,margin=dict(l=0,r=0,t=10,b=0),
            yaxis=dict(title="W/m²",color="#B87200"),
            yaxis2=dict(title="kW",color="#A83030",overlaying="y",side="right"),
            hovermode="x unified",
            paper_bgcolor="rgba(0,0,0,0)",plot_bgcolor="rgba(0,0,0,0)",
            legend=dict(orientation="h",yanchor="bottom",y=1.02,font=dict(color=MUTED,family="Inter")),
            font=dict(color=MUTED,family="Inter"))
        fig_d.update_xaxes(showgrid=False,color=MUTED)
        fig_d.update_yaxes(gridcolor=BORDER,color=MUTED)
        st.plotly_chart(fig_d,use_container_width=True)
    with t4:
        st.caption("ΔT = Forward − Return. Higher ΔT = more energy extracted per litre")
        st.plotly_chart(linechart(chart_df,
            ["temp_difference","temp_forward","temp_return"],
            [TEXT,"#A83030","#2E5EA0"],"°C"),use_container_width=True)

else:
    st.markdown('<div class="section-title">Solar & Environment</div>', unsafe_allow_html=True)
    c1,c2,c3,c4 = st.columns(4)
    c1.metric("Solar Irradiance", fmt(v.get("irradiance"),0,"W/m²"), help="TOA max ~1350 W/m²")
    ...
    with st.expander("📥 Raw data & export"):
        pivot = df.pivot_table(index="created_at",columns="sensor",values="value",aggfunc="last")             .reset_index().sort_values("created_at",ascending=False)
