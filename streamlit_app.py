...
Light theme, smooth auto-refresh, irradiance gauge added.
"""
...
<style>
  ...
  div[data-testid="metric-container"]{{background:{CARD};border:1px solid {BORDER};border-radius:10px;padding:12px 16px;}}
  div[data-testid="metric-container"] label{{font-size:.7rem;color:{MUTED};text-transform:uppercase;letter-spacing:.07em;}}
  div[data-testid="metric-container"] [data-testid="stMetricValue"]{{color:{TEXT};font-size:1.2rem;font-weight:600;}}
  ...
  .live-dot{{display:inline-block;width:8px;height:8px;border-radius:50%;background:{GREEN};margin-right:6px;}}
</style>
...
def semi(label, val, mn, mx, unit, color, sub="", warn=None):
    steps = [{"range": [mn, warn if warn else mx], "color": "#EEF2FF"}]
    if warn:
        steps.append({"range": [warn, mx], "color": "#FFF3CD"})
...
def linechart(df, sensors, colors, ylabel, height=270):
    ...
    fig.update_layout(
        ...
        yaxis_title=ylabel,
        legend=dict(orientation="h", yanchor="bottom", y=1.02, font=dict(size=10, color=MUTED)),
        hovermode="x unified",
        paper_bgcolor="rgba(0,0,0,0)", plot_bgcolor="rgba(0,0,0,0)",
...
# ── Smooth auto-refresh (no page blink) ───────────────────────
if auto_ref:
...
if st.session_state.view == "gauges":
    ...
    with f1:
        st.plotly_chart(semi("Flow rate", v.get("flow"), 0, 1, "m³/h", LBLUE,
                             "Fluid circulation"), use_container_width=True, config={"displayModeBar":False})
    with f2:
        st.plotly_chart(semi("Thermal power", v.get("power"), 0, 9.2, "kW", RED,
                             "Peak 9.2 kW @ 1000 W/m²"), use_container_width=True, config={"displayModeBar":False})
    with f3:
        ...
        st.plotly_chart(semi("Solar irradiance", irr, 0, 1000, "W/m²", irr_color,
                             f"{irr_sub} — design peak 1000 W/m²"), use_container_width=True, config={"displayModeBar":False})
    with f4:
    ...
    k2.metric("Energy Counter",   fmt(ef/1000 if ef else None,2,"kWh"), help="Cumulative meter Wh→kWh")
